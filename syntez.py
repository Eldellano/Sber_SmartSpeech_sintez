...
text_for_sintez = 'Hello! Это тестовый тест чтобы тестировать тесты!'
...
bin_lst = []
for i in resp.content:  # переводим в двоичный код
    # print(i, '||', format(i, '0{}b'.format(8)))
    # bin_lst.append(format(i, '0{}b'.format(8)))
    bin_lst.append(format(i, '0{}b'.format(8)))
#print(bin_lst)


bit_16 =[]
for i in range(0, len(bin_lst), 2):  # конкатинируем первый и второй элемент
    bit_16.append(bin_lst[i] + bin_lst[i+1])

out = []
for i in bit_16:
    #print(i)
    out.append(float(int(i, 2)))
# for i in bin_lst:
#     out.append(float(int(i, 8)))
# for i in out:
#     print(i, '||', type(i))


with open('result.wav', 'wb') as outfile:
    outfile.write(resp.content)
data, fs = sf.read('result.wav')
sd.play(data, fs)
sd.wait()
...
# воспроизведение
# sd.play(out, SAMPLE_RATE)  # воспроизведение
# sd.wait()
#sd.stop()
